 ...
 *  Lab 7 - Topological Path Planning and Execution
 *  
 *  Description:
 *    This code is used to control the robot, plan a path from the robot's starting postion to an ending position,
 *    and then assist the robot in navigating to the ending position. A 4x4 topological world map consisting of
 *    integers between 0 and 15 is used, with the integers representing various features such as walls, hallways,
 *    corners, junctions, and empty spaces; a specific feature is dependent upon the wall(s) around the space.  The starting
 *    and ending coordinates are also given to the robot by the user via pushbuttons.  An optimal path is
 *    generated using a wavefront expansion algorithm, which the robot uses to navigate the world map from it's
 *    starting position to the ending position.  While mavigating the world map, the robot uses algorithms from
 *    previous labs to follow walls and avoid obstacles.
...
void ConvertingMap() { //converting the map from 1-15s to 0s and 99s
  printMAP(); //print the map to the LCD
  while (STATE_current == convertMap) { //while it is in the convertMap state
    for (int y = 0; y < cols; y++) { //loop through the matrix
      for (int x = 0; x < rows; x++) {
        if (MAP[x][y] == 15) { //if the value is a 15
          MAP[x][y] = 99; //make it a 99
        }
        //if the value is one of the "U" variations and it is not the goal or the robot start
        else if (((MAP[x][y] == 14) || (MAP[x][y] == 13) || (MAP[x][y] == 11) || (MAP[x][y] == 7)) && ((MAP[x][y] != currentWave) || (MAP[x][y] != 20))) {
          MAP[x][y] = 99; //make it a 99
        }
        //if the cell only has one wall or is a hallway
        else if ((MAP[x][y] == 1) || (MAP[x][y] == 2) || (MAP[x][y] == 4) || (MAP[x][y] == 5) || (MAP[x][y] == 10) || (MAP[x][y] == 8)) {
          MAP[x][y] = 0; //make it a 0 (open)
        }
        //if the robot is going from top left to bottom right this will be used for the map
        //        else if ((MAP[x][y] == 6) ||  && (MAP[x][y + 1] == 10)) {
        //          MAP[x][y + 1] = 50;
        //          MAP[x][y] = 0;
        //        }
        //if the robot is going from bottom right to top left, if the value right of a 6 is a 10
        else if ((MAP[x][y] == 6) && (MAP[x][y + 1] == 10)) {
          MAP[x][y + 1] = 0; //make the value to the right a 0
          MAP[x][y] = 50; //make the current value a 50
        }
        //else if its an L and its not a 10
        else if (((MAP[x][y] == 3)  ||  (MAP[x][y] == 9) || (MAP[x][y] == 12)) && (MAP[x][y + 1] != 10)) {
          MAP[x][y] = 0; //make it a 0
        }
      }
    }
    ...
    if (Robot.keyboardRead() == BUTTON_MIDDLE) {
      ...
      printMAP(); //print map on LCD
      delay(500);
      STATE_current = makePath; //change state
      SetState(); //go to state machine
      break;
    }
  }
}
...
void makeMetricPath () {
  while (STATE_current == makePath) { //while state is makePath
    ...
    while (foundWave == true) {
        ...
        for (int x = 0; x < rows; x++) {
          if (MAP[x][y] == currentWave) {
            ...
            if (goalX < (rows - 1)) { // Checks the array bounds to the East
              ...
              else if (MAP[goalX + 1][goalY] == 50) { // checks if it is an L bracket
                MAP[goalX + 1][goalY] = currentWave - 1; //make next movement change the 50 so that it is part of the path
              }
            }
            ...
            if (goalY < (cols - 1)) { // Checks the array bounds to the North
              ...
              else if (MAP[goalX][goalY + 1] == 50) { // checks if it is an L bracket
                MAP[goalX - 1][goalY] = currentWave - 1; //make next movement change the 50 so that it is part of the path
              }
            }
          }
      ...
      currentWave--;
      if ((MAP[goalX + 1][goalY] == 20) || (MAP[goalX][goalY + 1] == 20) || (MAP[goalX - 1][goalY] == 20) || (MAP[goalX][goalY - 1] == 20)) { //if its at the robot start break out of the loop
...
void NavigateMetricPath() {
  ...
  int next_Direction = 1; //next direction is south
  int current_high = currentWave; //initialize where the robot path is
  int i = 0; //initialize i
  while (current_high < 98) {
    current_high = currentWave + i; // Every time, increment the current high
    next_Direction = current_facing;
    int Next_X = 0; 
    int Next_Y = 0;

    if (current_x > 0) { // Checks the array bounds to the West
      if ((MAP[current_x - 1][current_y] == current_high + 1) && (MAP[current_x - 1][current_y] != 99)) { // Checks if current space is next in the path
    ...
    if (current_y > 0) { // Checks the array bounds to the South
      if ((MAP[current_x][current_y - 1] == current_high + 1) && (MAP[current_x][current_y - 1] != 99)) { // Checks if current space is next in the path
    ...
    delay(movedelay + adjustdelay); //delay for a time that the robot moves about 18 inches
    //    CenterFollow(); //go to center follow to adjust the robot if need be
...
void updateLCD() {
  ...
  switch (STATE_current) {
    case convertMap:
      ...
      Robot.text("Converting Map", 5, 15);
      break;
    case setupPath:
      Robot.stroke(0, 0, 0); //print text
